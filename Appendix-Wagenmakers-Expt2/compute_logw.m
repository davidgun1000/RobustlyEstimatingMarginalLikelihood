function [logw]=compute_logw(prop_theta,data,num_subjects,num_trials,num_particles,num_randeffect,mean_thetalatent,covmat_thetalatent,mixprop_mean,mixprop_Sigma,mixprop_weight,i)
    ...
    param.num_randeffect=8; %the parameter proposal for \mu_{\alpha}
    param.theta_mu=[prop_theta(i,1:8)]; %the parameter proposal for the cholesky factor of covariance matrix \Sigma_{\alpha}
    chol_theta_sig2_temp=[prop_theta(i,9:44)]';
    %---------------------------------------------------------------
    ...
    chol_theta_sig2(8,1:8)=[chol_theta_sig2_temp(29:36,1)];
    ...
    log_prior_a=log_IG_PDF_used(param.a1,0.5,1)+log_IG_PDF_used(param.a2,0.5,1)+log_IG_PDF_used(param.a3,0.5,1)+log_IG_PDF_used(param.a4,0.5,1)+...
        log_IG_PDF_used(param.a5,0.5,1)+log_IG_PDF_used(param.a6,0.5,1)+log_IG_PDF_used(param.a7,0.5,1)+log_IG_PDF_used(param.a8,0.5,1);%compute the prior of a_1,...,a_8
    ...
    logw_den3=log(2^num_randeffect)+(num_randeffect-1+2)*log(chol_theta_sig2_temp(1,1))+(num_randeffect-2+2)*log(chol_theta_sig2_temp(3,1))+...
        ...
        (num_randeffect-8+2)*log(chol_theta_sig2_temp(36,1));
