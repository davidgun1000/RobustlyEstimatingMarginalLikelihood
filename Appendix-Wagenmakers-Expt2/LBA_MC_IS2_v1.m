...
parfor j=1:num_subjects
    ...
    n1=sum(id1);
    n2=N_i-n1;
    chol_theta_sig2=chol(param.theta_sig2,'lower'); %compute the cholesky factor of \Sigma_{\alpha}
    ...
    rnorm=rnorm';
    %--------------------------------------------
    rnorm_theta_b11=rnorm(:,1);
    rnorm_theta_b12=rnorm(:,2);
    rnorm_theta_b21=rnorm(:,3);
    rnorm_theta_b22=rnorm(:,4);
    rnorm_theta_A=rnorm(:,5);
    rnorm_theta_v1=rnorm(:,6);
    rnorm_theta_v2=rnorm(:,7);
    rnorm_theta_tau=rnorm(:,8);

    rnorm_theta=[rnorm_theta_b11,rnorm_theta_b12,rnorm_theta_b21,rnorm_theta_b22,rnorm_theta_A,rnorm_theta_v1,rnorm_theta_v2,rnorm_theta_tau];
    ...
    data_actualresp_repmat=repmat(data.actualresp{j,1}(:,1),N_i,1);
    [rnorm_theta_b_kron]=reshape_b(data_cond_repmat,data_actualresp_repmat,rnorm_theta_b11_kron,rnorm_theta_b12_kron,rnorm_theta_b21_kron,rnorm_theta_b22_kron); %choose the threshold particles to match with the stimulus and conditions of the experiment
    ...
    %effects
    %------------------------------
    lw=real(log(LBA_n1PDF_reparam_real_twoaccum(data_rt_repmat, rnorm_theta_A_kron, rnorm_theta_b_kron, rnorm_theta_v_kron, ones(N_i*num_trials(j,1),1),rnorm_theta_tau_kron)));
    ...
    xx=[param.theta_mu';chol_theta_sig2_1';chol_theta_sig2_2';chol_theta_sig2_3';...
        chol_theta_sig2_4';chol_theta_sig2_5';chol_theta_sig2_6';chol_theta_sig2_7';chol_theta_sig2_8']; % we need this to compute the mean of the proposal of the random effects
    cond_mean=mean_theta(j,1:8)'+covmat_theta(1:8,9:end,j)*((covmat_theta(9:end,9:end,j))\(xx-mean_theta(j,9:end)'));% computing the mean of the proposal of random effect
    cond_var=covmat_theta(1:8,1:8,j)-covmat_theta(1:8,9:end,j)*(covmat_theta(9:end,9:end,j)\covmat_theta(9:end,1:8,j));  %computing the covariance matrix of the proposal of the random effect
    chol_cond_var=chol(cond_var,'lower');
    rnorm1=cond_mean+chol_cond_var*randn(param.num_randeffect,n1);
    chol_covmat=chol(param.theta_sig2,'lower');
    rnorm2=param.theta_mu'+chol_covmat*randn(param.num_randeffect,n2);
    ...
    data_actualresp_repmat=repmat(data.actualresp{j,1}(:,1),num_particles_inc,1);
    [rnorm_theta_b_kron]=reshape_b(data_cond_repmat,data_actualresp_repmat,rnorm_theta_b11_kron,rnorm_theta_b12_kron,rnorm_theta_b21_kron,rnorm_theta_b22_kron); %choose the threshold particles to match with the stimulus and conditions of the experiment
    [rnorm_theta_v_kron]=reshape_v(data_response_repmat,rnorm_theta_v1_kron,rnorm_theta_v2_kron); %set the drift rate particles to match with the response
    %computing the log of the weights
        %----------------------------------------
    lw=real(log(LBA_n1PDF_reparam_real_twoaccum(data_rt_repmat, rnorm_theta_A_kron, rnorm_theta_b_kron, rnorm_theta_v_kron, ones(num_particles_inc*num_trials(j,1),1),rnorm_theta_tau_kron)));
    
    lw_reshape=reshape(lw,num_trials(j,1),num_particles_inc);
    logw_first=sum(lw_reshape);
    
    logw_second=(logmvnpdf([rnorm_theta_b11,rnorm_theta_b12,rnorm_theta_b21,rnorm_theta_b22,rnorm_theta_A,...
         rnorm_theta_v1,rnorm_theta_v2,rnorm_theta_tau],param.theta_mu,param.theta_sig2));
    logw_third=log(w_mix.*mvnpdf([rnorm_theta_b11,rnorm_theta_b12,rnorm_theta_b21,rnorm_theta_b22,rnorm_theta_A,...
         rnorm_theta_v1,rnorm_theta_v2,rnorm_theta_tau],cond_mean',chol_cond_var*chol_cond_var')+...
         (1-w_mix).*mvnpdf([rnorm_theta_b11,rnorm_theta_b12,rnorm_theta_b21,rnorm_theta_b22,rnorm_theta_A,...
         rnorm_theta_v1,rnorm_theta_v2,rnorm_theta_tau],param.theta_mu,param.theta_sig2));
