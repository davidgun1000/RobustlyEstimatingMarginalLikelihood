...
% The example of the input is given in the Matlab file 'LBA_Forstmann.mat',
% To run this program, you need to make sure that you have appropriate
% input as in 'LBA_Forstmann.mat'.
% The dataset is 'LBA_realdata.mat', it has data.response = 1 if response
...
tic
num_randeffect=7; %number of random effects
load('LBA_realdata.mat'); %load the dataset
num_subjects=19; %number of subjects
...
v_alpha=2;
%obtain proposal
load('LBA_Forstmann.mat'); %load the posterior MCMC draws of random effects \alpha and parameters \mu_{\alpha} and \Sigma_{\alpha}
length_draws=length(theta_latent_A_store); %compute the number of MCMC draws from the input file 'LBA_Forstmann.mat'
...
theta_latent_b3_temp=theta_latent_b3_store(length_draws-9999:end,:);
...
for j=1:num_subjects
    ...
    % \alpha_{b_3}, \alpha_A, \alpha_{v_11}, \alpha_{v_12}, \alpha_{v_21}, \alpha_{v_22},
    ...
    covmat_thetalatent(:,:,j)=cov(theta_latent); %computing the sample covariance matrix for the random effects, parameters \mu_{\alpha} and \Sigma_{\alpha}
    mean_thetalatent(j,:)=mean(theta_latent); %computing the sample mean for the random effects, parameters \mu_{\alpha} and \Sigma_{\alpha}
end
...
X=[theta_mu_temp,chol_theta_sig2_1,chol_theta_sig2_2,chol_theta_sig2_3,chol_theta_sig2_4,chol_theta_sig2_5,chol_theta_sig2_6,...
    chol_theta_sig2_7,log(a1_half),log(a2_half),log(a3_half),log(a4_half),log(a5_half),log(a6_half),log(a7_half)];
...
num_param=42; % the total number of parameters in the LBA model.
u=rand(IS_samples,1);
...
save('IS_realthreethreshold.mat','logw','log_marglik_boot','CPUtime');
