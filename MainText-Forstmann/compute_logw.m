function [logw]=compute_logw(prop_theta,data,num_subjects,num_trials,num_particles,num_randeffect,mean_thetalatent,covmat_thetalatent,mixprop_mean,mixprop_Sigma,mixprop_weight,i)
    ...
    v_alpha=2;
    %
    param.theta_mu=[prop_theta(i,1:7)]; %the parameter proposal for \mu_{\alpha}
    chol_theta_sig2_temp=[prop_theta(i,8:35)]'; %the parameter proposal for the cholesky factor of covariance matrix \Sigma_{\alpha}
    %---------------------------------------------------------------
    ...
    chol_theta_sig2(2,1:2)=chol_theta_sig2_temp(2:3,1);
    chol_theta_sig2(3,1:3)=chol_theta_sig2_temp(4:6,1);
    chol_theta_sig2(4,1:4)=chol_theta_sig2_temp(7:10,1);
    chol_theta_sig2(5,1:5)=chol_theta_sig2_temp(11:15,1);
    chol_theta_sig2(6,1:6)=chol_theta_sig2_temp(16:21,1);
    chol_theta_sig2(7,1:7)=chol_theta_sig2_temp(22:28,1);
    
    param.theta_sig2=chol_theta_sig2*chol_theta_sig2'; %the parameter proposal for \Sigma_{\alpha}
    %------------------------------------------------------------
    param.a1=exp(prop_theta(i,36)); %the parameter proposal for a_{1}
    param.a2=exp(prop_theta(i,37)); %the parameter proposal for a_{2}
    param.a3=exp(prop_theta(i,38)); %the parameter proposal for a_{3}
    param.a4=exp(prop_theta(i,39)); %the parameter proposal for a_{4}
    param.a5=exp(prop_theta(i,40)); %the parameter proposal for a_{5}
    param.a6=exp(prop_theta(i,41)); %the parameter proposal for a_{6}
    param.a7=exp(prop_theta(i,42)); %the parameter proposal for a_{7}   
    ...
    log_prior_sigma=logiwishpdf_used(param.theta_sig2,2*v_alpha*diag([1/param.a1;1/param.a2;1/param.a3;1/param.a4;1/param.a5;1/param.a6;1/param.a7]),v_alpha+num_randeffect-1,num_randeffect); % compute the log of the prior for \Sigma_{\alpha}
    log_prior_a=log_IG_PDF_used(param.a1,0.5,1)+log_IG_PDF_used(param.a2,0.5,1)+log_IG_PDF_used(param.a3,0.5,1)+log_IG_PDF_used(param.a4,0.5,1)+...
        log_IG_PDF_used(param.a5,0.5,1)+log_IG_PDF_used(param.a6,0.5,1)+log_IG_PDF_used(param.a7,0.5,1); %compute the prior of a_1,...,a_7
    ...
    logw_den2=log(1/param.a1)+log(1/param.a2)+log(1/param.a3)+log(1/param.a4)+log(1/param.a5)+log(1/param.a6)+log(1/param.a7);
    logw_den3=log(2^num_randeffect)+(num_randeffect-1+2)*log(chol_theta_sig2_temp(1,1))+(num_randeffect-2+2)*log(chol_theta_sig2_temp(3,1))+...
        ...
        (num_randeffect-6+2)*log(chol_theta_sig2_temp(21,1))+(num_randeffect-7+2)*log(chol_theta_sig2_temp(28,1));
