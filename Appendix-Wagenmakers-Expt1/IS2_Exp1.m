...
% The example of the input is given in the Matlab file 'LBA_Exp1.mat',
% To run this program, you need to make sure that you have appropriate
% input as in 'LBA_Exp1.mat'.
% The dataset is 'LBA_data_Exp1.mat', it has data.response = 1 if response
% is incorrect and response=2 if the response is correct, data.cond
% specifies the conditions of the experiments, data.rt contains the
% response times. 

num_randeffect=8; %number of random effects
load('LBA_Exp1.mat');  %load the posterior MCMC draws of random effects \alpha and parameters \mu_{\alpha} and \Sigma_{\alpha}
load('LBA_data_Exp1.mat'); %load the dataset
num_subjects=17; %number of subjects
...
tic
%obtain proposal
length_draws=length(theta_latent_A_store); %compute the number of MCMC draws from the input file 'LBA_Exp1.mat'
...
theta_latent_b1_temp=theta_latent_b1_store(length_draws-9999:end,:);
theta_latent_b2_temp=theta_latent_b2_store(length_draws-9999:end,:);
theta_latent_v11_temp=theta_latent_v11_store(length_draws-9999:end,:);
theta_latent_v12_temp=theta_latent_v12_store(length_draws-9999:end,:);
theta_latent_v21_temp=theta_latent_v21_store(length_draws-9999:end,:);
theta_latent_v22_temp=theta_latent_v22_store(length_draws-9999:end,:);
...
for j=1:num_subjects
    ...
    % \alpha_A, \alpha_{v_11}, \alpha_{v_12}, \alpha_{v_21}, \alpha_{v_22},
    % \alpha_{tau}, (2) followed by the parameters \mu_{\alpha}, and
    % cholesky factor (lower triangular matrix) of the covariance matrix
    % \Sigma_{\alpha}
    
    theta_latent=[theta_latent_b1_temp(:,j),theta_latent_b2_temp(:,j),...
        theta_latent_A_temp(:,j),theta_latent_v11_temp(:,j),theta_latent_v12_temp(:,j),theta_latent_v21_temp(:,j),theta_latent_v22_temp(:,j),theta_latent_tau_temp(:,j),...
    ...
    covmat_thetalatent(:,:,j)=topdm(covmat_thetalatent(:,:,j));
...
X=[theta_mu_temp,chol_theta_sig2_1,chol_theta_sig2_2,chol_theta_sig2_3,chol_theta_sig2_4,chol_theta_sig2_5,chol_theta_sig2_6,...
    chol_theta_sig2_7,chol_theta_sig2_8,log(a1_half),log(a2_half),log(a3_half),log(a4_half),log(a5_half),log(a6_half),log(a7_half),log(a8_half)];
options = statset('MaxIter',5000);
k=2;
mixtwo=fitgmdist(X,k,'Options',options,'Replicates',20);
...
save('IS2_Exp1.mat','logw','log_marglik_boot','CPUtime');
